...
function getPaletteGeneratorCardHelpersWindow() {
  return window as any;
}

function getDom() {
  return getPaletteGeneratorCardHelpersWindow().AppDom || {};
}

function getConstants() {
  return getPaletteGeneratorCardHelpersWindow().AppConstants || {};
}

function getGlobals() {
  return getPaletteGeneratorCardHelpersWindow().PaletteGeneratorLegacyGlobals || {};
}

function getCardsRuntime() {
  return getPaletteGeneratorCardHelpersWindow().PaletteGeneratorCardsRuntime || {};
}

export function initializePaletteGeneratorCardHelpers() {
  if (hasInitializedPaletteGeneratorCardHelpers) {
    return;
  }

  const runtimeWindow = getPaletteGeneratorCardHelpersWindow();

  runtimeWindow.syncCurrentPaletteFromDom = function syncCurrentPaletteFromDom() {
    Array.from(runtimeWindow.getColorCards?.() || []).forEach((card: any, index: number) => {
      card.dataset.index = String(index);
    });

    const globals = getGlobals();
    globals.currentPalette = runtimeWindow.getCurrentPaletteHexValues?.() || [];
    runtimeWindow.syncPaletteGeneratorStoreCurrentPalette?.(globals.currentPalette, {
      scope: "current-palette",
    });
    AppSharedColors.setPalette?.(globals.currentPalette, {
      source: "palette-generator",
    });
    runtimeWindow.refreshColorCardNames?.();
  ...
  runtimeWindow.getColorCards = function getColorCards() {
    return getDom().paletteContainer?.querySelectorAll(".color-card") || [];
  };

  runtimeWindow.isExplicitMonochromaticColorModeSelected =
  ...
  runtimeWindow.isExplicitComplementaryColorModeSelected =
    function isExplicitComplementaryColorModeSelected() {
      const globals = getGlobals();
      return (
        globals.paletteBaseMode === "color" &&
        globals.selectedColorPaletteType === "complementary"
      );
    };

  runtimeWindow.getColorModeBaseCardIndex = function getColorModeBaseCardIndex(
    totalCount = runtimeWindow.getColorCards?.().length || 0
  ) {
    const globals = getGlobals();
    const effectiveType =
      typeof runtimeWindow.getEffectiveColorPaletteType === "function"
        ? runtimeWindow.getEffectiveColorPaletteType(totalCount || globals.paletteSize)
        : globals.selectedColorPaletteType;

    return getCardsRuntime().getColorModeBaseCardIndex({
  ...
  runtimeWindow.getComplementaryRoleCardIndex = function getComplementaryRoleCardIndex(
    totalCount = runtimeWindow.getColorCards?.().length || 0
  ) {
    const globals = getGlobals();
    const effectiveType =
    ...
    return getCardsRuntime().getComplementaryRoleCardIndex({
      paletteBaseMode: globals.paletteBaseMode,
      effectiveType,
      totalCount,
    });
  ...
  runtimeWindow.isCardPinned = function isCardPinned(card: HTMLElement | null) {
    return card?.dataset.pinned === "true";
  };

  runtimeWindow.getCurrentPaletteCardEntries = function getCurrentPaletteCardEntries() {
    return Array.from(runtimeWindow.getColorCards?.() || [])
      .map((card: any, index: number) => {
        const hex = AppColorUtils.normalizeHexColor(
        ...
        return {
          ...
          index,
          hex,
          pinned: runtimeWindow.isCardPinned?.(card),
          regenerateLocked: card.dataset.regenerateLocked === "true",
        };
      })
      .filter((entry: any) => AppColorUtils.isValidHexColor(entry.hex));
  };

  runtimeWindow.getPinnedPaletteIndexes = function getPinnedPaletteIndexes() {
    const globals = getGlobals();
    const totalCount = runtimeWindow.getColorCards?.().length || 0;
    const effectiveType =
      typeof runtimeWindow.getEffectiveColorPaletteType === "function"
        ? runtimeWindow.getEffectiveColorPaletteType(totalCount || globals.paletteSize)
        : globals.selectedColorPaletteType;

    return getCardsRuntime().getPinnedPaletteIndexes({
  ...
  runtimeWindow.setCardColor = function setCardColor(card: HTMLElement, color: string) {
    const normalizedColor = AppColorUtils.normalizeHexColor(color);
    const overlayStyle = runtimeWindow.getAccessibleOverlayIconStyle?.(normalizedColor) || {
      color: "#000000",
    ...
    if (colorName) {
      runtimeWindow.applyAccessibleColorNameStyle?.(colorName, normalizedColor);
    }
  ...
  runtimeWindow.clamp = function clamp(value: number, min: number, max: number) {
    return Math.min(max, Math.max(min, value));
  };

  runtimeWindow.getRelativeLuminanceFromHex = function getRelativeLuminanceFromHex(hex: string) {
    return AppColorUtils.getRelativeLuminance(hex);
  };

  runtimeWindow.getContrastRatio = function getContrastRatio(hexA: string, hexB: string) {
    return AppColorUtils.getContrastRatio(hexA, hexB);
  };

  runtimeWindow.getAccessibleColorNameStyle = function getAccessibleColorNameStyle(
    backgroundHex: string
  ) {
    const normalized = AppColorUtils.normalizeHexColor(backgroundHex);
    const bgLuminance = runtimeWindow.getRelativeLuminanceFromHex?.(normalized) || 0;
    ...
    let mixAmount = 0.34;
    let candidate = AppColorUtils.mixHexColors(normalized, extremeColor, mixAmount);
    let contrast = runtimeWindow.getContrastRatio?.(candidate, normalized) || 0;

    while (contrast < contrastTarget && mixAmount < 0.92) {
      mixAmount += 0.06;
      candidate = AppColorUtils.mixHexColors(normalized, extremeColor, mixAmount);
      contrast = runtimeWindow.getContrastRatio?.(candidate, normalized) || 0;
    }
    ...
    const opacity =
      contrast >= 4.5 ? 0.8 : contrast >= contrastTarget ? 0.88 : 0.95;
  ...
  runtimeWindow.applyAccessibleColorNameStyle = function applyAccessibleColorNameStyle(
    colorNameElement: HTMLElement,
    backgroundHex: string
  ) {
    const style = runtimeWindow.getAccessibleColorNameStyle?.(backgroundHex);
    if (!style) {
      return;
    }
  ...
  runtimeWindow.getReadableTooltipTextColor = function getReadableTooltipTextColor(
    backgroundHex: string
  ) {
    return AppColorUtils.getReadableTextColor(backgroundHex);
  };

  runtimeWindow.getAccessibleOverlayIconStyle = function getAccessibleOverlayIconStyle(
    backgroundHex: string
  ) {
    const normalized = AppColorUtils.normalizeHexColor(backgroundHex);
    const accessibleStyle = runtimeWindow.getAccessibleColorNameStyle?.(normalized) || {
      textColor: "#000000",
    };
    const bgLuminance = runtimeWindow.getRelativeLuminanceFromHex?.(normalized) || 0;
  ...
  runtimeWindow.isDisallowedColor = function isDisallowedColor(color: string) {
    const constants = getConstants();
    return constants.DISALLOWED_COLORS?.has?.(AppColorUtils.normalizeHexColor(color)) || false;
  };

  runtimeWindow.getAdaptiveMinColorDistance = function getAdaptiveMinColorDistance(
    existingCount: number,
    attempt: number
  ) {
    const globals = getGlobals();
    const targetCount = Math.max(Number(globals.paletteSize) || 0, existingCount + 1);
    let baseDistance = 46;
  ...
  runtimeWindow.getRgbDistance = function getRgbDistance(colorA: string, colorB: string) {
    return AppColorUtils.getRgbDistance(colorA, colorB);
  };

  runtimeWindow.isColorTooCloseToExisting = function isColorTooCloseToExisting(
    candidateHex: string,
    ...
    const minDistance =
      runtimeWindow.getAdaptiveMinColorDistance?.(existingColors.size, attempt) || 0;

    for (const existingHex of existingColors) {
      if ((runtimeWindow.getRgbDistance?.(candidateHex, existingHex) || 0) < minDistance) {
        return true;
      }
    }
  ...
  runtimeWindow.getUniqueGeneratedColor = function getUniqueGeneratedColor(
    existingColors = new Set<string>()
  ) {
    const constants = getConstants();
    for (let attempt = 0; attempt < (constants.MAX_UNIQUE_COLOR_ATTEMPTS || 0); attempt += 1) {
      const candidate = AppColorUtils.normalizeHexColor(runtimeWindow.generateColor?.());

      if (runtimeWindow.isDisallowedColor?.(candidate)) {
        continue;
      }
      ...
      if (runtimeWindow.isColorTooCloseToExisting?.(candidate, existingColors, attempt)) {
        continue;
      }
  ...
  runtimeWindow.positionEditPickerAtButton = function positionEditPickerAtButton(
  ...
  runtimeWindow.openNativeColorPicker = function openNativeColorPicker(inputEl: HTMLInputElement) {
    try {
  ...
  runtimeWindow.createCardActionButton = function createCardActionButton(
    actionName: string,
    tooltipText: string
  ) {
  ...
  runtimeWindow.getPinButtonIconMarkup = function getPinButtonIconMarkup(isPinned: boolean) {
    if (isPinned) {
      return `
  ...
  runtimeWindow.getPinOverlayIconMarkup = function getPinOverlayIconMarkup(isPinned: boolean) {
    if (isPinned) {
      return `
      <span class="pin-overlay-icon-state pin-overlay-icon-state-filled is-visible">
        ${runtimeWindow.getPinButtonIconMarkup(true).replace(
          'class="pin-icon"',
    ...
    <span class="pin-overlay-icon-state pin-overlay-icon-state-outline is-visible">
      ${runtimeWindow.getPinButtonIconMarkup(false).replace(
        'class="pin-icon"',
        'class="pin-icon pin-overlay-icon"'
      )}
    </span>
  ...
  runtimeWindow.createCardPinButton = function createCardPinButton() {
    const button = document.createElement("button");
    button.type = "button";
  ...
  runtimeWindow.setActionButtonTooltipText = function setActionButtonTooltipText(
    button: HTMLElement,
    tooltipText: string
  ) {
  ...
  runtimeWindow.showButtonCopyFeedback = function showButtonCopyFeedback(
    button: HTMLElement,
    {
      defaultTooltipText,
      feedbackBg = null,
      ...
      feedbackBg?: string | null;
      feedbackTextColor?: string | null;
      durationMs?: number;
    } = {}
  ) {
    const constants = getConstants();
    ...
    return window.setTimeout(() => {
      tooltip.textContent =
        defaultTooltipText ?? constants.CARD_COPY_TOOLTIP_DEFAULT ?? "Copiar color";
  ...
  hasInitializedPaletteGeneratorCardHelpers = true;
}

export default initializePaletteGeneratorCardHelpers;
